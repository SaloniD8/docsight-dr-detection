...
#  Uses EfficientNet-B4 + Grad-CAM (model.py)
# ──────────────────────────────────────────────────────────────────────────────
...
try:
    ...
    traceback.print_exc()
    # Do NOT raise here — let the app start so /health can report the error


def numpy_to_base64(img_rgb):
    """Convert numpy RGB image → base64 PNG string."""
...
def predict_route():
    if model is None:
        error_detail = model_error if model_error else "Unknown error during model initialization."
        print(f"[predict] Model is None. Load error was: {error_detail}")
        return jsonify({
            "success": False,
            "error": f"Model not loaded. {error_detail}"
        }), 500
...
def health():
    if model is not None:
        ...
        return jsonify({
            ...
            "model": "EfficientNet-B4 DR",
            "error": model_error or "Unknown"
        }), 500
